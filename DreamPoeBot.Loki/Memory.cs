...
public class Memory : IDisposable
{
	...
	public Memory(Offsets offs, int pId)
	{
		...
		{
			throw new Exception(global::_003CModule_003E.smethod_8<string>(3349213354u), innerException);
		}
	}
	...
	public int GetModule(string name)
	{
		...
		int num = Process.Modules.Cast<ProcessModule>().First((ProcessModule m) => m.ModuleName == name).BaseAddress.ToInt32();
	...
	public byte ReadByte(long addr, params long[] offsets)
	{
		...
		{
			long num3 = offsets[i];
			num2 = ((i < offsets.Length - 1) ? ReadLong(num2 + num3) : ReadByte(num2 + num3));
		}
		return (byte)num2;
	}
	...
	[SuppressUnmanagedCodeSecurity]
	internal unsafe byte[] ReadMemOld(long addr, int count)
	{
		byte[] array = new byte[count];
		fixed (byte* lpBuffer = array)
		{
			if (WinApi.ReadProcessMemory(procHandle, (IntPtr)addr, lpBuffer, count, out var lpNumberOfBytesRead) && lpNumberOfBytesRead == count)
			{
				return array;
			}
		}
	...
	{
		int num = 0;
		string key = global::_003CModule_003E.smethod_8<string>(3454983806u);
	...
	internal LokiPoe.TerrainData.Struct46[] ReadTgtStructsArray<T>(long startAddress, int length) where T : struct
	{
		int size = MarshalCache<LokiPoe.TerrainData.Struct46>.Size;
		List<LokiPoe.TerrainData.Struct46> list = new List<LokiPoe.TerrainData.Struct46>();
		long num = startAddress + length * size;
		int num2 = 0;
		for (long num3 = startAddress; num3 < num; num3 += size)
		{
			num2++;
			if (num2 > 1000000)
			{
			...
			list.Add(FastIntPtrToStruct<LokiPoe.TerrainData.Struct46>(num3, size));
		}
		return list.ToArray();
	}

	internal LokiPoe.TerrainData.SubTileStruct[] ReadStructure48StructsArray<T>(long startAddress, int length, int size) where T : struct
	{
		List<LokiPoe.TerrainData.SubTileStruct> list = new List<LokiPoe.TerrainData.SubTileStruct>();
		long num = startAddress + length * size;
		int num2 = 0;
		for (long num3 = startAddress; num3 < num; num3 += size)
		{
			...
			list.Add(FastIntPtrToStruct<LokiPoe.TerrainData.SubTileStruct>(num3, size));
		}
		return list.ToArray();
	}

	internal Struct245InventoryWrapper[] ReadStructure245StructsArray<T>(long startAddress, long endAddress, int maxCountLimit = 400) where T : struct
	{
		int num = 0;
		string key = global::_003CModule_003E.smethod_8<string>(2951447760u);
		if (StructureSizesDictionary.TryGetValue(key, out var value))
		{
			num = value;
		}
		...
		{
			num = MarshalCache<Struct245InventoryWrapper>.Size;
			StructureSizesDictionary.Add(key, num);
		}
		List<Struct245InventoryWrapper> list = new List<Struct245InventoryWrapper>();
		...
		if (num2 >= 0L && num2 / num <= maxCountLimit)
		{
			...
			for (long num4 = startAddress; num4 < endAddress; num4 += num)
			{
				...
				list.Add(FastIntPtrToStruct<Struct245InventoryWrapper>(num4, num));
			}
			return list.ToArray();
		}
		return list.ToArray();
	...
	internal Struct242[] ReadStructure242StructsArray<T>(long startAddress, long endAddress, int maxCountLimit = 400) where T : struct
	{
		int num = 0;
		string key = global::_003CModule_003E.smethod_6<string>(3178652018u);
		if (StructureSizesDictionary.TryGetValue(key, out var value))
		{
			num = value;
		}
	...
	internal Struct243[] ReadStructure243StructsArray<T>(long startAddress, long endAddress, int maxCountLimit = 400) where T : struct
	{
		int num = 0;
		string key = global::_003CModule_003E.smethod_8<string>(1466155836u);
		if (!StructureSizesDictionary.TryGetValue(key, out var value))
		{
	...
	public List<long> ReadPointersArray(long startAddress, long endAddress, int offset = 8)
	{
		...
		int num = 0;
		for (long num2 = startAddress; num2 < endAddress; num2 += offset)
		{
			num++;
			if (num > 1000000)
			{
	...
	{
		int num = 0;
		string key = global::_003CModule_003E.smethod_6<string>(2806461858u);
		if (StructureSizesDictionary.TryGetValue(key, out var value))
		{
			num = value;
		}
	...
	public NativeStringWCustom IntptrToStructNativeStringWCustom(byte[] data)
	{
		GCHandle gCHandle = GCHandle.Alloc(data, GCHandleType.Pinned);
		try
		{
			return (NativeStringWCustom)Marshal.PtrToStructure(gCHandle.AddrOfPinnedObject(), typeof(NativeStringWCustom));
		}
		finally
		{
			gCHandle.Free();
	...
	public T[] IntptrToStructArray<T>(long start, int count, int structureSize) where T : struct
	{
		List<T> list = new List<T>();
	...
	public T[] IntptrToStructArray<T>(NativeVector vector, int structuresize) where T : struct
	{
		List<T> list = new List<T>();
	...
	public unsafe T IntptrToStruct<T>(byte[] data) where T : struct
	{
	...
	{
		int num = 0;
		string key = global::_003CModule_003E.smethod_6<string>(3078214250u);
		if (StructureSizesDictionary.TryGetValue(key, out var value))
		{
			num = value;
		}
		...
		Parallel.For(0, patterns.Length, delegate(int iPattern)
		{
			...
			for (int i = 0; i < exeImage.Length - num; i++)
			{
				if (CompareData(pattern, exeImage, i))
				{
					flag = true;
					address[iPattern] = i;
					DebugStr = DebugStr + global::_003CModule_003E.smethod_9<string>(926480812u) + iPattern + global::_003CModule_003E.smethod_6<string>(1762993969u) + (AddressOfProcess + i).ToString(global::_003CModule_003E.smethod_8<string>(2050146120u)) + global::_003CModule_003E.smethod_5<string>(29529409u) + i.ToString(global::_003CModule_003E.smethod_7<string>(1741115268u)) + Environment.NewLine;
					break;
				}
			}
			if (!flag)
			{
				DebugStr = DebugStr + global::_003CModule_003E.smethod_8<string>(1309752956u) + iPattern + global::_003CModule_003E.smethod_8<string>(2105284144u) + Environment.NewLine;
			}
		});
		return address;
	}
	...
	{
		int num = 0;
		string key = global::_003CModule_003E.smethod_8<string>(4240109685u);
		if (StructureSizesDictionary.TryGetValue(key, out var value))
		{
			num = value;
		}
	...
	{
		int num = 0;
		string key = global::_003CModule_003E.smethod_6<string>(719526080u);
		if (!StructureSizesDictionary.TryGetValue(key, out var value))
		{
	...
	{
		int num = 0;
		string key = global::_003CModule_003E.smethod_9<string>(1948181990u);
		if (!StructureSizesDictionary.TryGetValue(key, out var value))
		{
	...
	{
		int num = 0;
		string key = global::_003CModule_003E.smethod_8<string>(156683293u);
		if (StructureSizesDictionary.TryGetValue(key, out var value))
		{
			num = value;
		}
	...
	{
		int num = 0;
		string key = global::_003CModule_003E.smethod_9<string>(2479086196u);
		if (!StructureSizesDictionary.TryGetValue(key, out var value))
		{
