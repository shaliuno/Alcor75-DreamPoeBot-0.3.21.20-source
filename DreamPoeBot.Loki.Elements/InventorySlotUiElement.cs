...
public class InventorySlotUiElement : Element
{
			...
			if (_stashType != StashType.FragmentEldrich && _stashType != StashType.FragmentMaven)
			{
				...
				foreach (Element child in base.Children)
				{
					if (num != 0)
					{
						...
						int num4 = (int)child.Width / slotsize;
						int num5 = (int)child.Height / slotsize;
	...
	public Vector2i LocationTopLeft
	{
		get
		{
			...
			{
				num = base.X + base.DeltaX;
				num2 = base.Y + base.DeltaY;
				Element element = base.Parent;
				if (_stashType == StashType.FragmentEldrich || _stashType == StashType.FragmentMaven)
				{
					element = base.Children[0];
				}
				while (element.Address != 0L && element.IdLabel != global::_003CModule_003E.smethod_5<string>(3859111587u))
				{
					num += element.X + element.DeltaX;
					num2 += element.Y + element.DeltaY;
			...
			else
			{
				...
				while (element2.Address != 0L && element2.IdLabel != global::_003CModule_003E.smethod_8<string>(4144744542u))
				{
					num += element2.X;
					num2 += element2.Y;
